...
<template>
    <div class="swiper">
        <div class="swiper-wrapper">
            <div v-for="vergerBoost in vergersBoost.data" :key="vergerBoost.id"
                @click="navigateToDetail(vergerBoost.id)"
                class="swiper-slide w-full sm:w-1/4 md:w-1/4 lg:w-1/6 xl:w-1/6 p-1">
                <!-- Immobilier Card -->
                <div class="card">
                    <div class="aspect-square overflow-hidden">
                        <img :src="'/storage/' + vergerBoost.image1" :alt="vergerBoost.imageAlt"
            ...
            <div class="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="mt-10 grid grid-cols-2 gap-6 sm:grid-cols-4 sm:gap-4 lg:mt-16">
                    ...
                    <article v-for="verger in vergers" :key="verger.id" @click="navigateToDetail(verger.id)"
                        ...
                        <div class="card">
                            <div class="aspect-square overflow-hidden">
                                <img :src="'/storage/' + verger.image1" :alt="verger.imageAlt"
                                    class="h-full w-full object-cover transition-all duration-300 group-hover:scale-125">
                            </div>
                            <div class="absolute top-0 m-2 rounded-full ">
                                <p
                            ...
                            <div class="my-2 mx-auto flex w-10/12 flex-col items-start justify-between">
                                <p class="mr-3 text-sm font-semibold text-principal align-center">{{
                                    verger.prix }} <span class="text-secondaire mr-3">Fcfa</span>
                                </p>
                                ...
                                <div class="mb-2 flex">
                                    ...
                                    <p class="mr-3 text-secondaire  text-sm font-semibold truncate">
                                        {{
                                            verger.nom.substring(0, 20) }}</p>
                                </div>
                                <h3 class="text-sm text-principal pt-[-8%] w-full border-b-[1px] border-gray-200 ">
                                    {{ verger.region }} ,Senegal</h3>

                                .

                            </div>
                            <div v-if="verger.booster == 1"
