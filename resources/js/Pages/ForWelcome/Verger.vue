    ...
    <div class="swiper">
        ...
        <div class="swiper-wrapper grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6 mb-8">
            <div v-for="vergerBoost in vergersBoost.data" :key="vergerBoost.id"
                @click="navigateToDetail(vergerBoost.id)" class="swiper-slide w-full p-1 cursor-pointer">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <article v-for="verger in vergers" :key="verger.id" @click="navigateToDetail(verger.id)"
                        ...
                        <div class="p-6 flex-grow">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h3 class="text-xl font-bold mb-1 text-principal">
                                        {{ verger.nom.substring(0, 20) }}{{ verger.nom.length > 20 ? '...' : '' }}
                                    </h3>
                                    <p class="text-gray-600 mb-2">
                                        <i class="fas fa-map-marker-alt text-[#eb2d53] mr-2"></i>
                                        {{ verger.region }}, Sénégal
                                    </p>
                                </div>
                            </div>
                            ...
                            <div v-if="verger.status == 'accepter'" class="mt-6 ">
