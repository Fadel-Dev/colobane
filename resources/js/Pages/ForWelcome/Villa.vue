...
<template>
    <div class="swiper">
        ...
        <div class="swiper-wrapper grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6 mb-8">
            <div v-for="villaBoost in villasBoost.data" :key="villaBoost.id"
                @click="navigateToDetail(villaBoost.id)" class="swiper-slide w-full p-1 cursor-pointer">
            ...
            <div class="mx-auto max-w-screen-xl px-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <article v-for="villa in villas" :key="villa.id" @click="navigateToDetail(villa.id)"
                        ...
                        <div class="relative">
                            <img :src="'/storage/' + villa.image1" :alt="villa.imageAlt || 'Image de la villa'"
                        ...
                        <div class="p-6 flex-grow">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h3 class="text-xl font-bold mb-1 text-principal">
                                        {{ villa.nom.substring(0, 20) }}{{ villa.nom.length > 20 ? '...' : '' }}
                                    </h3>
                                    <p class="text-gray-600 mb-2">
                                        <i class="fas fa-map-marker-alt text-[#eb2d53] mr-2"></i>
                                        {{ villa.region }}, Sénégal
                                    </p>
                                </div>
                            </div>
                            ...
                            <div v-if="villa.status == 'accepter'" class="mt-6 ">
