...
export default {
    ...
    setup(props) {
        ...
        const regions = computed(() => {
            const allRegions = props.appartements.data.map(appartement => appartement.region);
            return ['Toutes les régions', ...new Set(allRegions)].filter(Boolean);
        });
        
        // Filtrage des appartements par région
        const filteredAppartements = computed(() => {
            if (selectedRegion.value === 'Toutes les régions') {
                return props.appartements.data;
            }
            return props.appartements.data.filter(appartement => appartement.region === selectedRegion.value);
        });
...
<template>
    <div class="swiper">
        <!-- Appartements Boostés -->
        <div class="swiper-wrapper grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6 mb-8">
            <div v-for="appartementBoost in appartementsBoost.data" :key="appartementBoost.id"
                @click="navigateToDetail(appartementBoost.id)" class="swiper-slide w-full p-1 cursor-pointer">
                <div class="card group relative bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 h-full flex flex-col">
                    ...
                    <div class="aspect-square overflow-hidden relative">
                        <img :src="'/storage/' + appartementBoost.image1" :alt="appartementBoost.imageAlt || 'Photo de l\'appartement'"
                            class="h-full w-full object-cover transition-all duration-500 group-hover:scale-110">
                        <div class="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                    </div>
                    <div class="p-4 space-y-2 flex-grow">
                        <p class="text-lg font-bold text-principal">
                            {{ appartementBoost.prix }} <span class="text-sm text-secondaire">Fcfa</span>
                        </p>
                        <p class="text-secondaire font-medium truncate">
                            {{ appartementBoost.nom.substring(0, 20) }}
                        </p>
                        <div class="flex items-center text-sm text-principal border-t border-gray-100 pt-2 mt-2">
                            <i class="fas fa-map-marker-alt mr-2"></i>
                            {{ appartementBoost.region }}, Senegal
                        </div>
                    </div>
        ...
        <section class="py-8" id="transparent">
            <div class="mx-auto max-w-screen-xl px-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <article v-for="appartement in filteredAppartements" :key="appartement.id" @click="navigateToDetail(appartement.id)"
                        ...
                        <div class="relative">
                            <img :src="'/storage/' + appartement.image1" :alt="appartement.imageAlt || 'Image de l\'appartement'"
                        ...
                        <div class="p-6 flex-grow">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h3 class="text-xl font-bold mb-1 text-principal">
                                        {{ appartement.nom.substring(0, 20) }}{{ appartement.nom.length > 20 ? '...' : '' }}
                                    </h3>
                                    <p class="text-gray-600 mb-2">
                                        <i class="fas fa-map-marker-alt text-[#eb2d53] mr-2"></i>
                                        {{ appartement.region }}, Sénégal
                                    </p>
                                </div>
                            </div>
                            ...
                            <div v-if="appartement.status == 'accepter'" class="mt-6">
                ...
                <!-- Pagination -->
                <div v-if="appartements.links && appartements.links.length > 1" class="flex justify-center mt-8 space-x-2">
                    <button 
                        v-for="link in appartements.links" 
                        :key="link.label"
                        :disabled="!link.url"
