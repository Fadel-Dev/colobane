    ...
    <AppLayout title="Dashboard">
        ...
        <div class="custom-grid">
            ...
            <div class="flex flex-wrap justify-center mt-20 text-gray-800">
                <div v-for="rv in rvs" :key="rv.id" @click="navigateToDetail(rv.id)" class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/4 p-2">
                    <div class="bg-secondaire rounded-2xl shadow-2xl relative mt-4">
                        <div class="w-full h-full">
                            <img :src="'/storage/' + rv.image1" :alt="rv.imageAlt" class="w-full h-full object-fill rounded-2xl shadow-lg">
                        </div>
                        ...
                        <div class="p-4">
                            <h3 class="text-lg font-semibold text-white">{{ rv.nom }}</h3>
                            <p class="text-gray-300">{{ rv.affaire }}</p>
                            <p class="text-principal text-lg">{{ rv.prix }}<span class="text-principal text-opacity-60"> Fcfa</span></p>
